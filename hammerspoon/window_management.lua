...
--                         ** Something Global **                       --
-- -----------------------------------------------------------------------
  require "window_management_db"

  createWinManagementDB()
  ...
  module.oldLocation = function ()
    local appID,x,y,w,h
    local app = hs.window.focusedWindow()
    appID, x, y, w,h = getOldLoc(app)
    
    if appID == nil then
      hs.alert.show("No previous location stored for " .. app:title())
      return
    end
    
    local oldLoc = Cell(x,y,w,h)
    app:move(oldLoc)
  end
